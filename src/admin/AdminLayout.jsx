...
export default function AdminLayout() {
  ...
  const [isRestricted, setIsRestricted] = useState(localStorage.getItem('al_mawaid_restricted') === 'true')
  ...
  useEffect(() => {
    const checkSession = async () => {
      ...
      if (session) {
        const fallback = session.user.user_metadata?.name || session.user.email || 'Admin'
        setAdminName(fallback)
        
        const { data: staff } = await supabase.from('staff').select('name, role').eq('user_id', session.user.id).maybeSingle()
        if (staff?.name) setAdminName(staff.name)
  ...
  const activeNAV = isRestricted ? NAV.filter(n => n.to === '/admin/settings') : NAV

  const filteredNav = activeNAV.filter(n => n.label.toLowerCase().includes(searchQuery.toLowerCase()))

  useEffect(() => {
    const isSettings = location.pathname === '/admin/settings' || location.pathname === '/admin/settings/'
    if (isRestricted && !isSettings) {
      ...
      >
        ...
        <header className="admin-header">
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            >
              <img src="/al-mawaid.png" alt="Logo" style={{ width: 20, height: 20, objectFit: 'contain' }} />
            </div>
            <div className="admin-nav-breadcrumb" style={{ display: 'flex', alignItems: 'center', gap: 8, color: 'rgba(255, 248, 225, 0.6)', fontSize: 13, fontWeight: 600 }}>
              ...
              {isRestricted && (
                <span style={{ marginLeft: 12, padding: '2px 8px', borderRadius: 20, background: 'rgba(212, 175, 55, 0.1)', color: 'var(--accent-primary)', fontSize: 10, border: '1px solid rgba(212, 175, 55, 0.3)', letterSpacing: '0.05em' }}>RESTRICTED</span>
              )}
            </div>
          </div>
        ...
        <div style={{ display: 'flex', flex: 1, overflow: 'hidden', position: 'relative' }}>
          ...
          <aside className={`admin-sidebar ${isSidebarOpen ? 'open' : ''}`}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', paddingRight: 10 }}>
              {activeNAV.map(({ to, label, Icon, end }) => (
